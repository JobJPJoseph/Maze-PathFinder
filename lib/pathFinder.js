    ...
    while (minQueue.length) {
        ...
        if (!visited.has(String(current))) {
            ...
            for (let i = 0; i < neighbors.length; i++) {
                ...
                minQueue.enqueue([...path, neighbor], obj.priority + 1, findC(neighbor, end)); // priority === steps taken

                // So now we need to optimize this because it makes unnessasary path that lead nowhere
                // We can not only test for steps but also the minumum distance
                    // So change the minQueue class to prioritize steps and minimum distance
